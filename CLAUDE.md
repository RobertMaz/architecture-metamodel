...
mvn -q -f analyzer/lst-extractor/pom.xml package   # собрать LST-экстрактор один раз — включает полку lst (OpenRewrite, Java+Kotlin, резолв URL/топиков)
...
- `analyzer/` — Kotlin/Maven: полки-источники (source — регулярки-fallback, lst — OpenRewrite LST Java+Kotlin с typed-резолвом URL/топиков (отдельный процесс `analyzer/lst-extractor`, classpath жертвы — из BOOT-INF/lib её jar), config, bytecode/ASM по `jar:` из repos.yml — аннотации + call-sites из тел методов (producers Kafka/Rabbit/StreamBridge, RestTemplate/WebClient; топики из LDC, констант и `@Value`-полей), runtime по `runtimeUrl:` — снимок Actuator mappings/env — и `traces:` — файл OTel-спанов (снять: `-javaagent:opentelemetry-javaagent.jar` + file/OTLP-экспорт), опционально springwolf (consumers Kafka/AMQP из fat-jar без запуска; отдельный сканер `analyzer/springwolf-scanner`, применим после его сборки), noir (второе мнение REST-in; бинарь OWASP Noir — `$NOIR_BIN`, `analyzer/noir/noir` или PATH) и jqassistant через адаптер `analyzer/jqassistant/extract.sh`, печатающий строки `TYPE|attr=value|...|source|confidence`) извлекают факты из репозиториев, реконсилятор сливает их в `tools/api-source/*.json` (v2); перед слиянием единый PlaceholderResolver (`core/Placeholders.kt`) резолвит `${...}` во всех фактах по цепочке application → профили → bootstrap → helm values → `config:` из repos.yml, нормализует env-суффиксы топиков и метит DLQ/retry (`channelRole`); Ktor-сервер (`server/`) — REST для UI: инвентарь, запуск анализа (после — сам зовёт `npm run gen`), отчёты, дифф, онбординг. Спека: `docs/superpowers/specs/2026-08-17-arch-analyzer-design.md`.
...
- `registry/systems.yml` — реестр систем (генерятся в `model/gen/systems/`); `registry/repos.yml` — привязка container-id к репозиторию (опционально `config:` — внешний конфиг вне репо, файл или каталог: ansible vars, helm values — для PlaceholderResolver). Владельцы — только в CODEOWNERS.
- `registry/clientlibs.yml` — клиентские api-либы: `groupId:artifactId → {container, jar?, path?}`; зависимость в pom/gradle даёт ребро в контейнер (0.6), либа профилируется в рёбра-операции (0.7): jar — ASM, path — сорцы через lst (fallback регулярки); байткод сервиса подтверждает реально вызываемые (0.85); кэш профилей — `workspace/_clientlibs/`.
...
- Прогоны анализа — очередь с одним воркером (`queued → running → done|failed` в `workspace/<id>/status.json`; зависшие после рестарта сервера помечаются failed). Источники контейнера (jar/runtimeUrl/traces) редактируются после онбординга: `PUT /api/containers/{id}/sources` или блок «Источники» на карточке. Перенос контейнера в другую систему (смена префикса id везде: repos.yml, api-source, workspace, алиасы, resolutions) — `POST /api/containers/{id}/move`, удаление со всеми артефактами — `DELETE /api/containers/{id}`; в UI — блок «Управление» на карточке. Рукописные ссылки на старый id подсветит `npm run check`.
