...
- `model/systems/<id>/<id>.c4` — **система анализатора целиком в одном файле** (контейнеры, api, сторы, каналы, связи и views); генерируется из `registry/systems.yml` + `tools/api-source/*.json`, руками не править — правки человека в `model/overrides.c4`. `model/gen/` — только догадки: `unknown/` (stub'ы), `observed/` (наблюдаемые контейнеры чужих систем), `_externals` + легаси v1 (`shop.orders.gen.c4`). CI падает, если `npm run gen` даёт дифф.
