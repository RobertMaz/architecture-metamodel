...
- `registry/clientlibs.yml` — клиентские api-либы: `groupId:artifactId → {container, jar?}`; зависимость в pom/gradle даёт ребро в контейнер (0.6), jar либы профилируется ASM в рёбра-операции (0.7), байткод сервиса подтверждает реально вызываемые (0.85); кэш профилей — `workspace/_clientlibs/`.
