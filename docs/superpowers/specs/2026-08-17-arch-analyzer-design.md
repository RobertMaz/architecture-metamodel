...
- контейнер генерируется внутри своей системы; принадлежность назначается при
  онбординге в `registry/repos.yml` (id содержит систему: `shop.orders`). Перенос в
  другую систему = операция `rename` движка (позже — кнопка в UI): id-aware замена по
  границам идентификатора во всех форматах (registry/*, overrides.c4, views,
  verified.json) + регенерация + `npm run check`. Голый sed не годится из-за подстрок
  (`shop.order` vs `shop.orders`). Единственное, что rename не чинит, — ссылки снаружи
  репо (чужие закладки/скрипты), поэтому с ростом внешних потребителей переносы
  дорожают, но не запрещаются;
