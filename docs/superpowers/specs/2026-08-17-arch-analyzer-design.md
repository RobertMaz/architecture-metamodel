...
```
model/
  10-shop.c4               руками: системы (L0) и существующее
...
- контейнер генерируется внутри своей системы; принадлежность решает человек один раз
  в `registry/repos.yml` (id содержит систему: `shop.orders`);
...
2. карточка контейнера: прикрепить источники (путь к сорцам / JAR / OpenAPI / URL
   запущенной апки) → «провести анализ» → живой статус полок → отчёт + дифф прогона;
