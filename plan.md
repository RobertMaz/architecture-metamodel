...
> **СТАТУС 2026-08-19: очередь работ (пп. 1–10) выполнена целиком.** Итог на petclinic:
> 51 → 68 связей чисто статикой, эндпоинты подтверждаются до пяти полок (c=1.0),
> Kotlin-паритет полный (demo.notifier), нераспознанный остаток после триажа — ноль.
> Клиентский e2e через UI прогнан. Дальше — по секции «Зоны роста» ниже, по мере
> появления триггеров.
...
## Зоны роста (зафиксированы 2026-08-19, после закрытия очереди)

Идём по ним не по порядку, а по триггерам — когда боевой ландшафт покажет необходимость.

| Зона | Триггер | Что делаем |
|------|---------|------------|
| **Боевой ландшафт** | доступ к рабочим репозиториям | `./up.sh ~/arch-data` (docs/data-root.md); настоящие метрики качества даст только он — первым, до всего остального |
| **Probe-слой: actuator/traces/eBPF** | доступ к поднятым сервисам / кластеру | роль — верификация, не новые факты: полки runtime/traces уже есть, докидываем eBPF-карту (Caretta/Coroot/Beyla), Otterize для Kafka; `verify.mjs --against=probe` снимает WARN «реконструкция без подтверждения» |
| **Межпроцедурный dataflow** | плейсхолдеры вида `{getUri()}` станут массовыми на боевом коде (на petclinic уже есть: genai) | пилот Joern (резерв ниже): CPG + `reachableBy`; kotlin2cpg пилотировать отдельно |
| **gRPC / GraphQL / WebSocket** | первый сервис с этим протоколом в ландшафте | proto-файлы — дешёвая полка уровня config; GraphQL-схемы аналогично |
| **Инфра-манифесты** | меши/managed-ресурсы в ландшафте | k8s ingress / Istio VirtualService / docker-compose → рёбра и хосты из инфры; terraform → сторы и очереди вне кода (S3/SQS/RDS) |
| **Схемы данных** | нужен точный shared-db детект / контракты сообщений | Flyway/Liquibase → таблицы в сторах; schema registry (Avro) → задействовать `MESSAGE_SCHEMA` (или убрать — решение висит с п. 2) |
| **Batch/файловые обмены** | в ландшафте появятся cron/Spring Batch/SFTP-интеграции | самая слепая зона статики; отдельный ресёрч источников (шедулеры, конфиги обменов) |
| **Non-JVM сервисы** | Python/Go/Node в ландшафте | noir уже кроет их REST-in; REST-out и messaging — свои полки по образцу lst |
| **Call graph / достижимость** | ASM перестанет хватать (транзитивные цепочки, DI) | SootUp (резерв ниже) |
| **Процесс подтверждений** | модель начинают читать люди | `verify.mjs --against=docs/owner` как регулярная практика; LLM-полка с корпоративным ключом (`registry/llm.yml`) — ревью пропусков |
